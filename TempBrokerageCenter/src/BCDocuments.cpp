...
bool BCDocuments::manageSubscriptionInOrderBook(bool isSubscript, const std::string& symbol, const std::string& targetID) const
{
    auto pos = m_orderBookBySymbol.find(symbol);
    if (m_orderBookBySymbol.end() != pos) {
        if (isSubscript) {
            ...
            pos->second->onUnsubscribeInOrderBook(targetID);
        return true;
    }
...
bool BCDocuments::manageSubscriptionInCandlestickData(bool isSubscript, const std::string& symbol, const std::string& username)
{
...
void BCDocuments::unregisterTargetFromOrderBooks(const std::string& targetID)
{
    ...
    if (m_orderBookSymbolsByTargetID.end() != pos) {
        for (const auto& symbol : pos->second) {
            m_orderBookBySymbol[symbol]->onUnsubscribeInOrderBook(targetID);
        }
...
void BCDocuments::unregisterTargetFromCandles(const std::string& targetID)
{
    ...
    auto pos = m_candleSymbolsByTargetID.find(username);
