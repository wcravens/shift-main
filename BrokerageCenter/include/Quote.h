...
struct Quote {
    ...
    Quote();
    Quote(std::string symbol, std::string clientID, std::string orderID, double price, int shareSize, ORDER_TYPE orderType); // for the server to receive
...
private:
    ...
    std::string m_clientID; ///<The client ID of quote
