...
The SHiFT front-end uses Thrift  an API for the Webclient's auth and market data feeds. When modifications regarding thrift are made, it is necessary to regenerate the Thrift files generated from shift_service.thrift.

The server code is generated in `backend/service/thrift/gen-cpp`, and the client code is generated to `backend/service/thrift/gen-php/client`. In the case of the php files, the `gen-php` directory must be moved to `frontend/service/thrift`.

The server and client files can be re-generated by running the `thrift-regen.sh` script, or individually as follows:
# gen-cpp

To regenerate the CPP Server code, execute the following commands:
...
To regenerate the PHP Client code, execute the following commands:
...
cp -r gen-php/ frontend/service/thrift/;
```
